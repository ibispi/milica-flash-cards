ui = { }
--------------------------------------------------------------------------------
	--adding sprites, sound or music for this github version should be done via imported music/sounds or sprites

	--do an "if my project" thing for each one of these: if it's your project do your thing with sprites and sounds
	--if it's not your project do the default individual sprite/sound/music import
...
------------------my program specific below-------------------------------------
--------------------------------------------------------------------------------

	ui.fadeOut = function (x, y)

	end

	ui.fadeIn = function (x, y)

	end

	ui.moveCanvas = function (x, y)

	end

	ui.setButtonEnlarge = function (x, y, tweenType)
	--stretches the button on hover
	end

	ui.setSFX = function (hover, click)

	end
	ui.setGFX = function (normal, hover, click, useAniparts)--can use aniparts
...
--[[--goes right by default here:
		...
		--goes left by default here:
		if tooltipX - _width_*1.3 >= 0 then
			tooltipX = tooltipX - (_width_ + ui.tooltip.settings.cursorOffset.x*2)
		end
		...
		if ui.tooltip.settings.colors.background[4] ~= 0 then
			setColorOld(ui.tooltip.settings.colors.background)
		...
		if ui.tooltip.settings.colors.outline[4] ~= 0 and ui.outlineEnabled == true then
			setColorOld(ui.tooltip.settings.colors.outline)
		...
		if ui.tooltip.settings.colors.font[4] ~= 0 then
			...
			setColorOld(ui.tooltip.settings.colors.font)
...
ui.clear = function ()
--acts like love.graphics.clear() except only when you have to clear a canvas from buttons
end
...
ui.addButton = function (content, permaClickEnabled, notificationVariables)
...
ui.addArrowButton =
{
	--there is a ui.variablesAffectedByArrow table...
	--there is a point variable that is spent...

	left = function (variableName, minNumber, limitedPointVariable, tableOfAffectedVariables)
		...
		limitedPointVar = limitedPointVariable, tableOfAffectedVars = tableOfAffectedVariables})
		end

	end,

	right = function (variableName, maxNumber, limitedPointVariable, tableOfAffectedVariables)
...
ui.addLabel = function (content, underline, highlightVar, highlightColor)
	...
	highlightVar = highlightVar, highlightColor = highlightColor})
...
ui.addVariableLabel = function (variableName, underline, highlightVar, highlightColor)
						...
						if _G[c.elements[q].variableName]==nil then
							...
							if c.elements[q].type == "arrowLeft" then
								if tonumber(_G[actualSubTable][c.elements[q].variableName]) > tonumber(c.elements[q].minNumber) then
									...
									if c.elements[q].limitedPointVar ~= nil then
										_G[c.elements[q].limitedPointVar] = _G[c.elements[q].limitedPointVar] + 1
										updateSkillPoints()
									end
	...
	if ui.canvasTable[canvasName]~=nil then
		...
		if c.elements[1]~= nil then
			...
			--  if rowCount~= nil then
					...
					if c.elements[q].type == "button" and dontDrawLine == false then
						...
						if ui.color[1][typeOfColorTable][4] ~= 0 then
							setColorOld(ui.color[1][typeOfColorTable])
						...
						if ui.color[2][typeOfColorTable][4] ~= 0 and ui.outlineEnabled == true then
							setColorOld(ui.color[2][typeOfColorTable])
							love.graphics.setLineWidth(ui.outlineThickness)
						...
						if ui.font.color[typeOfColorTable][4] ~= 0 then
							love.graphics.setFont(ui.font.name)
							setColorOld(ui.font.color[typeOfColorTable])
					...
					elseif c.elements[q].type == "sliderHorizontal" and dontDrawLine == false then
							...
							if ui.color[3][typeOfColorTable][4] ~= 0 then
								...
								setColorOld(ui.color[3][typeOfColorTable])
							...
							if ui.color[4][typeOfColorTable][4] ~= 0 and ui.outlineEnabled == true then
								...
								setColorOld(ui.color[4][typeOfColorTable])
								love.graphics.setLineWidth(ui.outlineThickness)
					...
					c.elements[q].type == "arrowRight") and dontDrawLine == false then
							...
							if typeOfColorTable == "hover" then
								...
								ui.hoveringOverRPGVariables = copy(c.elements[q].tableOfAffectedVars)
						...
						elseif c.elements[q].type == "textBox" and dontDrawLine == false then
							...
							if ui.textBoxColors[typeOfColorTable][4] ~= 0 then
								setColorOld(ui.textBoxColors[typeOfColorTable])
								love.graphics.rectangle('fill', currentX+bo, startY+bo,
								_width_-bo, _height_-bo, _width_*rectAngle, _height_*rectAngle)
							end
					...
					elseif c.elements[q].type == "label" and dontDrawLine == false then
						...
						if checkTableForVariable(ui.hoveringOverRPGVariables, c.elements[q].highlightVar) then
							setColorOld(c.elements[q].highlightColor)
						else
							setColorOld(ui.font.color.normal)
						end
					...
					elseif c.elements[q].type == "checkBox" and dontDrawLine == false then
						...
						if isItTrue == true then
							setColorOld(ui.checkBoxMarkColor[typeOfColorTable])
							love.graphics.setLineWidth(ui.outlineThickness)
			...
			if c.backgroundOutlineColor[4] ~= 0 and ui.outlineEnabled == true then
				...
				setColorOld(c.backgroundOutlineColor)
					...
					if ui.verticalSliderStats[canvasName].alignment == "right" then
						...
						if rowsPerScreen < ui.verticalSliderStats[canvasName].totalRows then
							...
							if ui.colorsVerticalSlider[canvasName][1][typeOfColorTable][4] ~= 0 then
								...
								setColorOld(ui.colorsVerticalSlider[canvasName][1][typeOfColorTable])
							...
							if ui.colorsVerticalSlider[canvasName][2][typeOfColorTable][4] ~= 0 and ui.outlineEnabled == true then
								...
								setColorOld(ui.colorsVerticalSlider[canvasName][2][typeOfColorTable])
								love.graphics.setLineWidth(ui.outlineThickness)
							...
							if ui.colorsVerticalSlider[canvasName][3][typeOfColorTable][4] ~= 0 then
								...
								setColorOld(ui.colorsVerticalSlider[canvasName][3][typeOfColorTable])
							...
							if ui.colorsVerticalSlider[canvasName][4][typeOfColorTable][4] ~= 0 and ui.outlineEnabled == true then
								...
								setColorOld(ui.colorsVerticalSlider[canvasName][4][typeOfColorTable])
								love.graphics.setLineWidth(ui.outlineThickness)
			...
			if ui.shouldTheToolTipBeDrawn.yesOrNo == true then
				ui.tooltipToBeDrawn = ui.shouldTheToolTipBeDrawn
